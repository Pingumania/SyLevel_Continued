...
do
	...
	title:SetText(P.Name..": Colors")

	local backdrop = P._BACKDROP
	...
	local methods = {
		...
		"Low Green, Yellow, Red High > 350",
		"Red Green, Yellow, Green High > 350",
		"Class Coloring",
        "Quality Coloring"
	}
	
	local colorDropdown = CreateFrame("Button", P.Name.."OptFColorDropdown", self, "UIDropDownMenuTemplate")
	...
	do
		local DropDown_OnClick = function(self)
			PingumaniaItemLevelDB.FilterSettings.colorFunc = self.value
			P:CallOptionCallbacks()
			
			P:SetColorFunc(self.value)
		...
		local UpdateSelected = function(self)
			UIDropDownMenu_SetSelectedID(colorDropdown, PingumaniaItemLevelDB.ColorFunc)
		end
	...
	local title = P.createFontString(self)
	title:SetPoint("BOTTOMLEFT", box, "TOPLEFT", 8, 0)
	title:SetText("Item colors")
	box.title = title

	local bTitle = P.createFontString(self)
	...
	local gTitle = P.createFontString(self)
	...
	local rTitle = P.createFontString(self)
	...
	local Reset_OnClick = function(self)
		local row = self:GetParent()
		Swatch_Update(row.swatch, nil, P:ResetColor(row.id))
	...
	for i=0, 7 do
		...
		local swatch = P.createColorSwatch(row)
		...
		local blueLabel = P.createEditBox(row)
		...
		local greenLabel = P.createEditBox(row)
		...
		local redLabel = P.createEditBox(row)
		...
		local nameLabel= P.createFontString(row)
